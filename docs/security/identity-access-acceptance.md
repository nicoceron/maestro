...
| Fortify | Headless normalized-email login, public/invited registration, reset, verification, ten-minute password/passkey confirmation, encrypted TOTP setup/confirm/recovery/disable with a ten-minute pending-setup expiry and replay-resistant five-minute login challenge, and official WebAuthn passkey login/confirmation/management routes are present under `/api/v1/auth` | Scaffolded; registration currently leaks new-vs-existing state through `201`/`422` and session/cookie divergence, so `IDA-REG-001` is failing until the uniform unauthenticated `202` flow lands; mandatory-role MFA/grace, full security notifications/audit, and recovery governance also remain |
...
Snapshot gaps that the test ledger deliberately exposes: public registration still enumerates new versus existing normalized emails through status/field/session-cookie differences; mandatory owner/administrator MFA enrollment and grace enforcement, complete security notifications, Sanctum PAT expiry, invitation resend/supersession, immutable security audit/outbox, ownership transfer, membership removal, and support-access flows are absent. Platform-specific 30-minute idle/eight-hour absolute sessions are not yet a separate runtime profile. Implemented controls now include verified-email owner onboarding, invitation-bound registration, HMAC-derived account limiters, uncompromised-password checking, reset invalidation of database sessions, session-registry rows, and PATs, trusted browser boundaries, fragment credential scrubbing, forced PostgreSQL RLS for memberships/invitations/user-session metadata, ten-minute recent confirmation and pending TOTP setup expiry, post-confirmation-only recovery-code disclosure, replay-resistant five-minute TOTP challenge, exact-origin WebAuthn passkeys, and user-facing session inventory/revocation with stale-record pruning and eight-hour idle/30-day absolute limits. These are implementation observations, not accepted exceptions for the remaining target gaps.
...
| `IDA-REG-001` | Public verified-email owner signup and invitation-bound signup grant no tenant authority before server onboarding/acceptance | `REGISTER-E001`–`REGISTER-E003`, `VERIFY-E001`–`VERIFY-E004` |
...
1. Public registration is an intentional owner-onboarding entry point. For both new and existing normalized emails it returns the same unauthenticated generic `202` verification guidance, with no distinguishable field errors, session/auth cookie, redirect, timing class, or notification claim. A new address creates only a pending, unverified global user; it grants no tenant authority until the email is verified and the server-controlled onboarding action creates the studio plus owner membership. Invitation-bound registration remains a distinct safe branch.
