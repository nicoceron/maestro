...
| Sanctum first-party cookie authentication | Sanctum 4.3, `statefulApi()`, `web` guard, credentialed CORS, trusted host/origin enforcement, and security headers are configured | Scaffolded; session inventory and absolute lifetime remain |
...
| Fortify | Headless dependency/provider/config and normalized-email login, registration, reset, verification, confirmation, and current-user routes are present under `/api/v1/auth`; TOTP/passkeys are not enabled | Scaffolded; public registration conflicts with the invitation-first target below |
| Invitations | Tenant-scoped list/create/revoke, JSON-body preview/accept, fragment-delivered and scrubbed browser credentials, digest-scoped RLS, normalized-email atomic acceptance, and non-accepting invited registration are present | Scaffolded; resend, full audit/outbox, and broader lifecycle remain |
...
| MFA, passkeys, session inventory/revocation, platform support access | Not implemented | Planned next gaps |
...
Snapshot gaps that the test ledger deliberately exposes: Fortify public registration is enabled; TOTP/passkeys and user-facing session inventory/revocation are not enabled; session idle lifetime defaults to 120 minutes without an absolute lifetime; Sanctum PAT expiry is `null`; and invitation resend, full security audit/outbox, ownership transfer, membership removal, and support-access flows are absent. Implemented controls now include HMAC-derived login and password-reset account limiter keys, Laravel's uncompromised-password check, password-reset invalidation of database sessions and PATs, trusted host/origin enforcement, security headers, fragment-delivered and immediately scrubbed invite/reset credentials, and forced PostgreSQL RLS for memberships and invitations. These are implementation observations, not accepted exceptions for the remaining target gaps.
...
1. Public registration is disabled for the initial invitation-first release. A public owner-onboarding flow MAY be added as a separate rate-limited use case that atomically creates a pending user and pending studio, then activates both only after email verification.
2. A valid invitation can begin account creation without revealing whether an account already exists. The invited address is server-bound and cannot be edited in the registration request.
3. Password registration creates `pending_verification`; it grants no membership access. Password policy is at least 12 characters, uncompromised-password checked, confirmed, and not dependent on composition tricks. Hash using the framework-selected Argon2id or bcrypt work factor and rehash on successful authentication when parameters change.
4. Verification links use Laravel temporary signed URLs, expire after 60 minutes, and are bound to user ID plus email hash. Resend is limited to 6/minute per user and 20/hour per IP and returns the same `202` shape.
5. Successful verification is idempotent, marks the exact current address verified, rotates the session, and resumes invitation acceptance if one is pending. A link for an old/pending-replaced email is rejected.
...
- Standard sessions expire after 8 hours idle and 30 days absolute; remembered sessions expire after 30 days absolute. Platform sessions expire after 30 minutes idle and 8 hours absolute and cannot be remembered. Absolute age is enforced server-side, not only by cookie expiry.
...
- Logout calls the framework logout operation, invalidates the server session, and regenerates the CSRF token. “Log out all devices” revokes every other session and personal token. Session inventory shows device label, coarse location, created/last-active time, and current marker without exposing IDs/cookies.
...
3. Create/resend returns a uniform `202` with the tenant invitation ULID and safe tenant-local status to authorized callers. It may say “invitation queued” whether the address is a new user, current member, or has another pending invite. Tenant-local duplicate/member state can be shown only from records the caller is already authorized to list, never from global account lookup.
...
7. Reuse of an accepted, expired, revoked, or superseded token returns the same generic `410` problem. Concurrent acceptance yields exactly one membership and one accepted event. A required `Idempotency-Key` retains the original safe response for 24 hours for an identical authenticated request; replay without the same key cannot use the token.
