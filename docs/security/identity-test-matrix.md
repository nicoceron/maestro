...
| REGISTER-E001 | `POST /api/v1/auth/register` without invitation, new vs existing normalized email | identical `202` and exact generic body, no redirect, same cookie names, unchanged guest session ID; only new creates one unverified user and encrypted after-commit verification delivery; existing name/password/user and notification count remain unchanged |
| REGISTER-E002 | `POST /api/v1/auth/register` with valid `invitation_token` | exact generic `202`, guest remains unauthenticated; matching new email creates only pending-verification user; no verification, invitation mutation, membership, role selection, or onboarding until explicit sign-in |
| REGISTER-E003 | Invitation-bound request with changed email, invalid/expired/revoked/accepted token, or existing normalized email | same `202` body/headers/cookies/timing and no semantic field error; no duplicate or changed user, verification delivery, invitation consumption, or membership |
| REGISTER-E004 | Registration validation, timing, and limits | passphrase of 12+ characters succeeds without composition rules; syntax/confirmation/compromise failures are account-independent `422`; every accepted path hashes within >=300 ms; sixth email+IP attempt/hour and 21st IP attempt/hour are `429`; limiter keys are HMACed and contain no raw email |
| REGISTER-E005 | Next registration success | client ignores server-provided success copy and shows fixed generic guidance; ordinary registration and the post-`202` path make no current-user probe, auth assumption, or onboarding redirect; an invitation may probe once before form rendering only to route an already-authenticated recipient; token remains confined to the scrubbed invitation session, sign-in is explicit, and keyboard focus moves to the result heading |
