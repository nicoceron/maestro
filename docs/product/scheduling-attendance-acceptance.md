...
| SCH-VIEW-001..005 | Public-safe projection, manager/teacher/billing projections, day/week/month/agenda Filament calendar, scoped filters, mobile/keyboard styling | Calendar query/context/resources, `ScheduleCalendar`, page tests, Playwright keyboard/axe/mobile checks and Linux snapshot | Scaffolded; populated production-volume and multi-role browser proof remain |
...
- expanded populated desktop/tablet/mobile visual regression and multi-role keyboard/WCAG journeys beyond the current empty-state calendar, teaching, and template gate;
