...
- Laravel: SQLite runs 100 tests with 94 passed, 1,162 assertions, and six PostgreSQL-only skips. A disposable PostgreSQL 18 database runs all 100 tests / 1,247 assertions with the restricted `maestro_runtime` role, including the five RLS integration tests and invitation acceptance after the `SECURITY DEFINER` finalizer is deliberately re-owned by that non-superuser/non-`BYPASSRLS` role. This covers default-deny and cross-tenant membership, invitation, user-session, audit, and delivery boundaries. The SQLite suite covers the full invitation lifecycle, including membership-, recent-confirmation-, input-validation-, and mutation-role-policy-before-quota ordering, create quotas of 20/hour per inviter across studios and 100/day per studio, resend limits, declared limiter windows and `Retry-After`, bounded terminal delivery failure, uniform registration, token-scoped invitation access, browser-boundary controls, reset invalidation, TOTP/passkey hardening, active-session inventory/revocation, and the existing studio/People/household policy coverage. The local PHP runtime lacks `ext-redis`, so these local suites use the array cache; Redis remains a CI application-path gate.
- Next.js: 11 Vitest files / 70 tests pass; ESLint passes and the production build passes. The duplicate authenticated studio shell, route gate, fixture dashboard, and invitation-management UI are removed. Legacy `/studio/{studio}/*` links are temporary redirects to `/manage/studio/{studio}` and render no Next.js authenticated application. The remaining credential, onboarding, and account-security pages predate the architecture decision and stay explicitly transitional until their Filament replacements preserve registration, invitation acceptance, TOTP, passkey, and session-management behavior; Next.js is not accepted as an authenticated product surface.
- Filament: the authenticated `/manage` panel normalizes login email, shares keyed account/IP login limits, registers the new database session before redirect, admits every active studio role while preserving resource-level policy boundaries, and exposes tenant-scoped Team Invitations and People Directory resources. The invitation surface has policy-derived role options, rate-limited current-password/recent-passkey confirmation, shared HMAC-keyed create/resend quotas, and guarded create/resend/revoke actions. The People tracer lists only the active studio, permits owner/administrator/office lead-student creation, normalizes contact email, creates the student profile atomically, and gives billing read-only directory access. Tests cover active/suspended tenant access, teacher panel access, login normalization/limiting/session registration, invitation tenant isolation/lifecycle/security, People tenant isolation, role-gated creation, and lead profile persistence.
