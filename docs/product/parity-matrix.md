...
- Identity: headless Fortify login/logout/reset/verification, Maestro-owned public registration, ten-minute password/passkey confirmation, encrypted TOTP setup/recovery with ten-minute pending-setup expiry, post-confirmation-only recovery-code disclosure, and a replay-resistant five-minute challenge, official exact-origin WebAuthn passkeys, safe passkey/current-user metadata, database session inventory/revocation with stale-record pruning and idle/absolute limits, JSON-body studio invitations, fragment credential scrubbing, and two-path onboarding are scaffolded. Public and invitation-bound registration now use one generic unauthenticated `202` for new, existing, and semantically unusable invitation cases; registration does not log in or rotate the CSRF session, only an eligible new identity receives the encrypted queued verification notification, every semantic path performs password-hash work inside the timebox, and HMAC-keyed account/IP limits apply. Passwords are minimum 12 characters, confirmed, and uncompromised checked without mixed-case/number/symbol composition requirements, so passphrases remain valid. Password-reset invalidation, trusted browser boundaries, and membership/invitation/user-session RLS are covered. Mandatory privileged-role MFA/grace, the platform session profile, invitation resend/supersession, security notifications, and immutable audit/outbox remain incomplete. Continue against [Laravel 13 Fortify's contracts](https://laravel.com/docs/13.x/fortify) and the [stable identity acceptance IDs](../security/identity-access-acceptance.md#12-stable-identity-control-ids).
- Laravel: SQLite runs 77 tests with 72 passed, 759 assertions, and five PostgreSQL-only skips. PostgreSQL 18 passes all 77 tests / 816 assertions, including uniform-registration behavior under the restricted runtime, restricted-role membership/invitation/user-session RLS, token-scoped invitation access, browser-boundary controls, reset invalidation, TOTP/passkey hardening, active-session inventory/revocation, and the existing studio/People/household policy coverage.
- Next.js: 13 Vitest files / 74 tests pass; ESLint passes with zero warnings and the production build passes. The live identity transport smoke also passes against Laravel with cookie/CSRF registration and login, current-user lookup, onboarding, tenant membership lookup, and logout. Registration renders fixed generic post-`202` guidance without a follow-up identity probe; only invitation landings may probe before rendering to route an already-authenticated recipient. Fragment credential scrubbing, invitation preview/acceptance, TOTP setup/challenge/recovery, passkey login/management, recent confirmation, and device/session management are covered by focused frontend and Laravel tests.
