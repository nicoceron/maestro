...
- Identity: headless Fortify registration/login/logout/reset/verification, ten-minute password/passkey confirmation, encrypted TOTP setup/recovery with ten-minute pending-setup expiry, post-confirmation-only recovery-code disclosure, and a replay-resistant five-minute challenge, official exact-origin WebAuthn passkeys, safe passkey/current-user metadata, database session inventory/revocation with stale-record pruning and idle/absolute limits, JSON-body studio invitations, fragment credential scrubbing, and two-path onboarding are scaffolded. Password compromise checks, reset invalidation, trusted browser boundaries, and membership/invitation/user-session RLS are covered. Public registration still has a confirmed `201`/`422`/session-cookie account-enumeration blocker and must move to uniform unauthenticated `202`; mandatory privileged-role MFA/grace, the platform session profile, invitation resend/supersession, security notifications, and immutable audit/outbox also remain incomplete. Continue against [Laravel 13 Fortify's contracts](https://laravel.com/docs/13.x/fortify) and the [stable identity acceptance IDs](../security/identity-access-acceptance.md#12-stable-identity-control-ids).
- Laravel: SQLite runs 71 tests with 67 passed, 667 assertions, and four PostgreSQL-only skips. PostgreSQL 18 passes all 74 tests / 756 assertions, including restricted-role membership/invitation/user-session RLS, token-scoped invitation access, browser-boundary controls, reset invalidation, TOTP/passkey hardening, active-session inventory/revocation, and the existing studio/People/household policy coverage.
- Next.js: 13 Vitest files / 70 tests pass; ESLint passes with zero warnings and the production build passes. The live identity transport smoke also passes against Laravel with cookie/CSRF registration and login, current-user lookup, onboarding, tenant membership lookup, and logout. Fragment credential scrubbing, invitation preview/acceptance, TOTP setup/challenge/recovery, passkey login/management, recent confirmation, and device/session management are covered by focused frontend and Laravel tests.
